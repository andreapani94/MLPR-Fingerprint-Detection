...
def plot_bayer_error(prior_logodds, model_dcf_map: dict[str, tuple[list[float], list[float]]], title: str=None):
    plt.figure()
    for model in model_dcf_map:
        plt.plot(prior_logodds, model_dcf_map[model][0], label=f'DCF ({model})', linewidth=2)
        plt.plot(prior_logodds, model_dcf_map[model][1], label=f'min DCF ({model})', linewidth=2)
    plt.ylim([0, 1.1])
    plt.xlim([-3, 3])
    plt.legend()
